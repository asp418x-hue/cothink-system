...
start_btop() {
    ...
    if DISPLAY="$DISPLAY" wmctrl -l | grep "btop_pip" &>/dev/null; then
    ...
    local win_id=""
    for i in {1..30}; do
        win_id=$(DISPLAY="$DISPLAY" wmctrl -l | grep "btop_pip" | awk '{print $1}' | head -n 1 || true)
        if [[ -n "$win_id" ]]; then
            break
        fi
        sleep 0.1
    ...
    DISPLAY="$DISPLAY" wmctrl -i -r "$win_id" -b add,above
    
    # 2. Make Sticky (visible on all workspaces)
    DISPLAY="$DISPLAY" wmctrl -i -r "$win_id" -b add,sticky
    
    # 3. Position and Resize: x=750, y=320, width=600, height=400
    DISPLAY="$DISPLAY" wmctrl -i -r "$win_id" -e 0,750,320,600,400
    
    # 4. Remove borders/decorations dynamically via Motif hints
    DISPLAY="$DISPLAY" xprop -id "$win_id" -f _MOTIF_WM_HINTS 32c -set _MOTIF_WM_HINTS "0x2, 0x0, 0x0, 0x0, 0x0"
    
    # 5. Bring window to top focus / raise it
    DISPLAY="$DISPLAY" xdotool windowraise "$win_id"
...
stop_btop() {
    ...
    local win_id
    win_id=$(DISPLAY="$DISPLAY" wmctrl -l | grep "btop_pip" | awk '{print $1}' || true)
    if [[ -n "$win_id" ]]; then
        for id in $win_id; do
            DISPLAY="$DISPLAY" xdotool windowkill "$id" 2>/dev/null || true
        done
